...
AI generates a migration plus `desc/sql/user.sql` schema snapshot (PostgreSQL):
...
**Step 3: AI automatically executes framework generation command**

```bash
# 1. After migrate up, generate Model from datasource
pzero gen

# 2. Generate only a specific api (skips model)
...
```
Descriptor files (api/proto) + datasource config → pzero gen → Complete framework code
```
...
- schema snapshots in `desc/sql/` (not gen input); migrations in `desc/sql_migration/`

✅ **One command handles code generation**
- `pzero gen` // Scan api/proto, and generate model from DB when model-datasource is enabled
- `pzero gen --desc desc/api/user.api` // Only generate user.api (skips model)
- `pzero gen --desc desc/proto/user.proto` // Only generate user.proto (skips model)
