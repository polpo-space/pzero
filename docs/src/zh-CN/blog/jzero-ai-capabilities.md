...
AI 根据需求自动生成 migration 与 `desc/sql/user.sql` schema snapshot（PostgreSQL）：
...
```bash
# 1. 应用 migration 后，从 datasource 生成 Model
pzero gen

# 2. 仅生成指定 api（会跳过 model）
...
```
描述文件 (api/proto) + datasource 配置 → pzero gen → 完整的框架代码
```
...
- schema snapshot 放在 `desc/sql/`（不喂 gen）；migration 放在 `desc/sql_migration/`

✅ **一条命令搞定代码生成**
- `pzero gen` // 扫描 api/proto，并在开启 model-datasource 时从数据库生成 model
- `pzero gen --desc desc/api/user.api` // 仅生成 user.api（跳过 model）
- `pzero gen --desc desc/proto/user.proto` // 仅生成 user.proto（跳过 model）
