...
import (
	...
	"github.com/polpo-space/pzero/cmd/pzero/internal/command/template/templatebuild"
	"github.com/polpo-space/pzero/cmd/pzero/internal/command/template/templateinit"
	"github.com/polpo-space/pzero/cmd/pzero/internal/command/version"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if config.C.Template.Init.Output == "" {
			...
			if config.C.Template.Init.Remote != "" && config.C.Template.Init.Branch != "" {
				config.C.Template.Init.Output = filepath.Join(home, ".pzero", "templates", "remote")
			} else {
				config.C.Template.Init.Output = filepath.Join(home, ".pzero", "templates", version.Version)
			}
		}
...
var templateBuildCmd = &cobra.Command{
	...
	Long:  `Build your current project to template and save them into ${HOME}/.pzero/templates/local.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		if config.C.Template.Build.Output == "" {
			home, _ := os.UserHomeDir()
			config.C.Template.Build.Output = filepath.Join(home, ".pzero", "templates", "local", config.C.Template.Build.Name)
		}
