...
import (
	...
	"github.com/polpo-space/pzero/cmd/pzero/internal/pkg/execx"
)
...
func Run(cmd *cobra.Command, hookAction, hooksName string, hooks []string) error {
	...
	if os.Getenv("PZERO_HOOK_TRIGGERED") == "true" {
		return nil
	}

	if os.Getenv("PZERO_FORKED") == "true" && hooksName == "global" && hookAction == "Before" {
		return nil
	}
	...
	for _, v := range hooks {
		...
		output, err := runOutputFn(v, wd, "PZERO_HOOK_TRIGGERED=true")
		if err != nil {
	...
	if len(hooks) > 0 && hookAction == "Before" && hooksName == "global" {
		...
		// 设置环境变量，防止无限递归
		env := append(os.Environ(), "PZERO_FORKED=true")
