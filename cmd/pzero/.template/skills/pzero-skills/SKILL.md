...
2. **Generate api code**: `pzero gen --desc desc/api/user.api`
...
3. **Apply migrations** (development: `pzero migrate up`, production: auto in `cmd/server.go`)
4. **Generate model**: `pzero gen --desc desc/sql/users.sql`

**Remote Datasource Mode** (schema from live database):
1. **Create migration files** in `desc/sql_migration/` (xx.up.sql & xx.down.sql) ⚠️
2. **Apply migrations** (development: `pzero migrate up`, production: auto in `cmd/server.go`)
3. **Generate model**: `pzero gen`
...
```
myproject/
├── .pzero.yaml       # CLI config: code generation, ⚠️ migrate settings
├── desc/
...
- **Code generation**: Run `pzero gen --desc` before implementing logic
...
2. Create a project: `pzero new myapi --frame api`
...
- **Upstream documentation**: [docs.jzero.io](https://docs.jzero.io)
- **GitHub repository**: [polpo-space/pzero](https://github.com/polpo-space/pzero)
