...
pzero supports multiple ways to generate models from SQL schemas. The generated models provide comprehensive CRUD methods with type-safe field constants and caching support.
...
- **Development**: Use `pzero migrate` commands with `.pzero.yaml`
...
3. **Generate models**: Run `pzero gen --desc desc/sql/table.sql`
4. **Apply migrations**:
   - Development: Use `pzero migrate up` with `.pzero.yaml`
...
- **Use `.pzero.yaml` for generation config** - Centralize settings
...
2. **Apply migrations to database**:
   - Development: Use `pzero migrate up` with `.pzero.yaml`
   - Production: Automatic via code in `cmd/server.go`
3. **Generate models from database**: Run `pzero gen`
...
- **Use `.pzero.yaml` for datasource config** - Centralize connection settings
...
pzero automatically generates `internal/model/model.go` that registers all models:
...
pzero generates these methods by default for each model:
...
- **Use `.pzero.yaml` for generation configuration** - Centralize your generation settings
...
| **Model generation** | `pzero gen --desc desc/sql/x.sql` | `pzero gen` |
