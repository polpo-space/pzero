...
2. **Development**: Use `pzero migrate` commands with `.pzero.yaml`
...
### Development Mode: pzero migrate Command

Use during development for quick testing:

```bash
pzero migrate up              # Apply pending migrations
pzero migrate down            # Rollback last migration
pzero migrate up 4            # Apply specific number of migrations
pzero migrate goto 3          # Go to specific version
pzero migrate version         # Check current version
```

**Requires**: `.pzero.yaml` configuration (see below)
...
var serverCmd = &cobra.Command{
    ...
    Run: func(cmd *cobra.Command, args []string) {
        ...
        m, err := migrate.NewMigrate(c.Sqlx.SqlConf)
        logx.Must(err)
        defer m.Close()
        logx.Must(m.Up())
...
**Enable with**: Add `sourceAppendDriver: true` to `.pzero.yaml` (development) OR use `migrate.WithSourceAppendDriver(true)` in code (production)
...
### Development Mode (.pzero.yaml)

Required for `pzero migrate` commands:

```yaml
# .pzero.yaml (development only)
migrate:
...
pzero gen --datasource="root:password@tcp(127.0.0.1:3306)/mydb"
...
### Do I need .pzero.yaml in production?

**NO!** Production uses code-based migration with `etc/etc.yaml`. `.pzero.yaml` is development-only.

### Should I use pzero migrate or code migration?

- **Development**: `pzero migrate` - quick testing, easy rollback
